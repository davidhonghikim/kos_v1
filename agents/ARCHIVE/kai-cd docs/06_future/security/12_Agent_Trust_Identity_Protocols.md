...
class SecureMessagingSystem {
  private identityManager: AgentIdentityManager;
  private trustEngine: TrustScoringEngine;
  private messageLog: Map<string, SignedMessage> = new Map();
  ...
  async sendMessage(
    ...
    const signature = await crypto.subtle.sign(
      'Ed25519',
      privateKey,
      new TextEncoder().encode(messageJson)
    );
    ...
    // Log message
    this.messageLog.set(messageId, signedMessage);

    return signedMessage;
  }

  async verifyMessage(message: SignedMessage): Promise<MessageVerificationResult> {
    try {
      // Get sender identity
      const senderIdentity = await this.getAgentIdentity(message.from);
      if (!senderIdentity) {
        return {
          valid: false,
          trusted: false,
          trustLevel: 0,
          reason: 'Sender identity not found'
        };
      }
      ...
      // Check trust level
      const trustLevel = await this.trustEngine.getTrustScore(message.from);
      const isTrusted = trustLevel >= 0.5; // Minimum trust threshold

      return {
        valid: true,
        trusted: isTrusted,
        trustLevel,
        reason: isTrusted ? 'Message verified and trusted' : 'Message verified but sender not trusted'
      };
    } catch (error) {
      return {
        valid: false,
  ...
  async getMessageHistory(agentId: string, limit: number = 100): Promise<SignedMessage[]> {
